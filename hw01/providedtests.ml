...
let provided_tests : suite = [
  ...
  Test ("Problem4-3 Harder Custom Tests", [
  ...
  Test ("Problem4-4 Hardest Custom Tests", [
    ("optimize1", assert_eqf (fun () -> interpret ctxt3 (optimize e4)) 1L);
    ("optimize2", assert_eqf (fun () -> interpret ctxt3 (optimize e5)) 9L);
    ("optimize3", (fun () -> try ignore (interpret ctxt3 (optimize e6)); failwith "bad interpret" with Not_found -> ()));
    ("optimize4", assert_eqf (fun () -> interpret ctxt3 (optimize e7)) 0L);
    ("optimize5", assert_eqf (fun () -> interpret ctxt3 (optimize e8)) (-162L));
  ]);
