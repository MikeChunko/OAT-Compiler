...
let legal_gep (nt:tid -> ty) (sty:ty) (tag:ty) : bool =
  let rec ptrtoi8 : ty -> ty = function
    ...
    | t -> t
  in let rec flatten_ty : ty -> ty list = function
    | Struct ts -> List.(concat @@ map flatten_ty ts)
    | t -> [t]
  in is_prefix (flatten_ty @@ ptrtoi8 sty) (flatten_ty @@ ptrtoi8 tag)
...
let interp_prog {tdecls; gdecls; fdecls} (args:string list) : sval =
  ...
  in let rec interp_cfg (k, blocks:cfg) (locs:locals) (c:config) : config * sval =
    ...
    | (_, Store (t, os, od))::insns, _ ->
       ...
       let c' = match vd with
         | VPtr p ->
            if effective_tag nt p <> t then raise IncompatTagDeref
            else store_ptr c p (MWord vs)
         | VUndef -> raise UndefPtrDeref
         | VInt _ -> failwith "non-vptr arg for load"
       in interp_cfg ({k with insns}, blocks) locs c'
    | (u, Call (t, ofn, ato))::insns, _ ->
       ...
       in let args = List.map2 (interp_op locs) ats aos in
    ...
    | (u, Gep (Ptr t, o, os))::insns, _ ->
       ...
       in let vs = List.map (interp_op locs I64) os in
  ...
  and interp_call (ty:ty) (fn:gid) (args:sval list) (c:config) : config * sval =
    if List.mem fn runtime_fns then runtime_call ty fn args c else
    let {f_param; f_cfg} = try List.assoc fn fdecls
                       with Not_found -> failwith @@ "interp_call: undefined function " ^ fn
    in if List.(length f_param <> length args) then
  ...
  in let ptrs, heap = List.fold_right mkarg ("LLINTERP"::args) ([],[]) in
