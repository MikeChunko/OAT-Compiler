...
let better_layout (f:Ll.fdecl) (live:liveness) : layout =
  ...
  let rec gen_rig (block:Ll.block) (live:liveness) (rig:rig_fact) : rig_fact =
    ...
    let rec add_to_rig (live:UidS.t) (acc:UidS.t) (rig:rig_fact) : rig_fact =
      ...
      | Some uid -> print_string uid;
        ...
        (*let rig' = UidM.add uid acc rig in*)
  ...
  let rec gen_coloring (rig:rig_fact) (coloring:coloring) (k:int) (num_spill:int) : coloring =
    ...
    (* Debug method *)
    let rec print_cardinality (lst: (uid * int) list ) : unit =
      match lst with
  ...
  (* Should hopefully be all I have to change *)
  (* Strat: Create a UidM that holds info to act as our RIG (key is a list of other UIDs)
            Simplistically solving it, creating a new UidM where the key holds the color (register #) of that UID
            Any register # > size(pal) gets spilled. *)
  let allocate lo uid =
    let loc =
    try
      let used_locs = UidSet.fold (fun y -> LocSet.add (List.assoc y lo)) (live.live_in uid) LocSet.empty in
